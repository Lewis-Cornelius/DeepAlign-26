param(
    ...
    [string]$Mode = "All",
    [string]$SwdPath = "data",
    [string]$Stage1Config = "config/deepalign_initial_claim_strict_ssl_stage1.yaml",
    [string]$Stage2Config = "config/deepalign_initial_claim_strict_pair_stage2.yaml",
    [string]$Stage3Config = "config/deepalign_initial_claim_strict_selfmine_stage3.yaml",
    [string]$BaselineCheckpoint = "checkpoints/initial_claim_self_audio_pretrain/best_model.pt",
    [string]$Stage1Checkpoint = "checkpoints/initial_claim_strict_ssl_stage1/best_model.pt",
    [string]$Stage2Checkpoint = "checkpoints/initial_claim_strict_pair_stage2/best_model.pt",
    [string]$Stage3Checkpoint = "checkpoints/initial_claim_strict_selfmine_stage3/best_model.pt",
    [string]$SelfMinedDir = "results/self_mined_initial_claim_strict_stage2",
    [string]$ResultsDir = "results/initial_claim_strict_audio_only",
    [string]$Device = "cuda"
)
...
function Invoke-Stage1 {
    Invoke-CheckedPython "Stage 1: strict same-audio SSL fine-tune from current best baseline" @(
...
function Invoke-Gate1 {
    ...
    Invoke-ResultGate -Label "Gate 1 threshold: <100 ms and >70% AR@50" -CsvPath $Csv -MaxMaeMs 100 -MinAr50Pct 70 -ExpectedPairs 8
}

function Invoke-Stage2 {
    Invoke-CheckedPython "Stage 2: strict same-lied pair training from current best self-audio checkpoint" @(
        "-u", "-m", "dis_alignment.cli", "train",
        "--config", $Stage2Config,
        "--swd-path", $SwdPath,
        "--device", $Device
...
function Invoke-Gate2 {
    ...
    Invoke-ResultGate -Label "Gate 2 threshold: <60 ms and >90% AR@50" -CsvPath $Csv -MaxMaeMs 60 -MinAr50Pct 90 -ExpectedPairs 8
}
...
function Invoke-Smoke {
    Invoke-CheckedPython "Smoke: Stage 1 one-epoch strict SSL" @(
        "-u", "-m", "dis_alignment.cli", "train",
        "--config", $Stage1Config,
        "--swd-path", $SwdPath,
        "--device", $Device,
        "--epochs", "1",
        "--samples-per-epoch", "8",
        "--output-dir", "checkpoints/initial_claim_strict_smoke_stage1",
        "--alignment-eval-every-n-epochs", "0"
    )
    Invoke-CheckedPython "Smoke: Stage 2 one-epoch strict pair training" @(
        "-u", "-m", "dis_alignment.cli", "train",
        "--config", $Stage2Config,
        "--swd-path", $SwdPath,
        "--device", $Device,
        "--epochs", "1",
        "--samples-per-epoch", "8",
        "--resume-from", "checkpoints/initial_claim_strict_smoke_stage1/best_model.pt",
        "--output-dir", "checkpoints/initial_claim_strict_smoke_stage2",
